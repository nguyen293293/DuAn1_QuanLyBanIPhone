...
public class DongSanPhamService implements ChungServices<DongSanPhamViewModle> {
    ...
    public String add(DongSanPhamViewModle t, Object... obj) {
        boolean isSave = dongSanPhamRepository.add(new DongSanPham("", genmahd(), t.getTen()));
        return isSave ? "Thêm thành công" : "Thêm thất bại";
    }

    private String genmahd() {
        List<DongSanPham> list = dongSanPhamRepository.getList();
        if (list.size() == 0) {
            return "DSP001";
        } else {
            int num = list.size() + 1;
            return "DSP" + (String.format("%03d", num));
        }
    }
